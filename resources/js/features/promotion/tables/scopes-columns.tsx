    ...
    return [
        {
            ...
            header: ({ column }) => (
                <DataTableColumnHeader
                    ...
                    title={i18n.t(
                        'management/promotions:scope.label.scope_type',
                    )}
                />
            ),
            cell: ({ row }) => {
                ...
                const map: Record<string, string> = {
                    ...
                    building: i18n.t(
                        'management/promotions:scope.label.building',
                    ),
                    floor: i18n.t('management/promotions:scope.label.floor'),
                    room_type: i18n.t(
                        'management/promotions:scope.label.room_type',
                    ),
        ...
        },
        {
            ...
            header: i18n.t('management/promotions:scope.label.floor'),
            cell: ({ row }) =>
                opts.resolvers?.floor?.((row.original as ScopeRow).floor_id) ??
                (row.original as ScopeRow).floor_id ??
                '-',
        },
        {
        ...
        {
            accessorKey: 'room_id',
            header: i18n.t('management/promotions:scope.label.room'),
            cell: ({ row }) =>
                opts.resolvers?.room?.((row.original as ScopeRow).room_id) ??
                (row.original as ScopeRow).room_id ??
                '-',
        },
        {
            id: 'actions',
            ...
            cell: ({ row }) => {
                const it = row.original as ScopeRow;
                return (
