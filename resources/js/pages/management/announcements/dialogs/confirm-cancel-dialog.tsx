  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>
            {t('management.notifications.confirm_cancel_title', 'Cancel Schedule')}
          </DialogTitle>
          <DialogDescription>
            {t(
        ...
        <DialogFooter className="gap-2 sm:gap-2">
          ...
          <Button
            onClick={() => {
              if (!item) return;
              router.visit(
                route('management.announcements.cancel_schedule', { announcement: item.id }),
                { method: 'post', preserveScroll: true, onSuccess: onClose, onFinish: onClose },
              );
            }}
          >
            <XIcon className="mr-2 h-4 w-4" />
            {t('management.notifications.cancel_schedule_now', 'Cancel schedule')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
