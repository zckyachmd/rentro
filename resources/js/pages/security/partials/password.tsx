...
export default function PasswordTab() {
  ...
  const onSubmitPassword = (e: React.FormEvent) => {
    ...
    passwordForm.patch(route('security.password.update'), {
      ...
      onError: () => toast.error('Gagal memperbarui password. Periksa input Anda.'),
    });
  };
      ...
      <CardContent>
        <form onSubmit={onSubmitPassword} className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            <Button type="submit" disabled={passwordForm.processing}>Simpan</Button>
            <Button type="button" variant="secondary" onClick={() => passwordForm.reset()}>
              Reset
            </Button>
          </div>
        </form>
