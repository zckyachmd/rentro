  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {typeof item?.title === 'string' ? item?.title : t('notifications.title', 'Notifications')}
          </DialogTitle>
          <DialogDescription>
            {item?.created_at ? (
              <time dateTime={item.created_at} title={formatDate(item.created_at, true)} className="text-muted-foreground text-xs">
                {formatTimeAgo(item.created_at)}
              </time>
            ) : null}
          </DialogDescription>
        </DialogHeader>
        <div className="whitespace-pre-wrap text-sm">{typeof item?.message === 'string' ? item?.message : ''}</div>
