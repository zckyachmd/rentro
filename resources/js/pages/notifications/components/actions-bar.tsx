  ...
  return (
    <div className="flex items-center gap-2">
      <ToggleGroup
        type="single"
        value={filter}
        onValueChange={(v) => v && onFilterChange(v as 'unread' | 'all')}
        size="sm"
        variant="outline"
      ...
      >
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <ToggleGroupItem value="unread" aria-label={t('notifications.unread', 'Unread')}>
              <BellDot className="h-4 w-4" />
            </ToggleGroupItem>
          </TooltipTrigger>
          <TooltipContent>{t('notifications.unread', 'Unread')}</TooltipContent>
        </Tooltip>
      </ToggleGroup>
      <Tooltip>
        <TooltipTrigger asChild>
